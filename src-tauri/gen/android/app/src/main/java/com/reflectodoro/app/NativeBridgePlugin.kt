...
class NativeBridgePlugin(private val activity: Activity) : Plugin(activity) {
    ...
    companion object {
        ...
        // --- Encryption at rest (see encryptFields/decryptFields below and
        // src-tauri/src/crypto.rs) ---
        private const val CRYPTO_TAG = "Reflectodoro/Crypto"
        private const val ANDROID_KEYSTORE = "AndroidKeyStore"

        // The alias this device's field-encryption key lives under. Changing
        // it (or generating over it) orphans every already-encrypted row --
        // the ciphertext stays, the key to read it does not.
        private const val FIELD_KEY_ALIAS = "reflectodoro_field_key"
        private const val FIELD_TRANSFORMATION = "AES/GCM/NoPadding"
        private const val FIELD_KEY_BITS = 256

        // Must match crypto.rs's MARKER byte for byte: both sides treat its
        // absence as "this value is legacy plaintext, pass it through".
        private const val FIELD_MARKER = "enc1:"

        // GCM's standard IV width, and the full 128-bit authentication tag.
        // The IV is prepended to the ciphertext in the stored blob -- it is
    ...
    /** Android's half of encryption at rest (see src-tauri/src/crypto.rs).
     *
     * Desktop keeps the key in the OS credential store and does the cipher
     * work in Rust; that isn't possible here, because an Android Keystore key
     * is non-exportable by design -- the raw bytes never leave the Keystore,
     * so the AES-256-GCM operation has to happen on this side, against a key
     * Rust only ever refers to by alias. That non-exportability is also why
     * Android is the strongest of the four platforms here: another app cannot
     * read this key even with the same user's device unlocked, which is not
     * true of the Windows or Linux credential stores.
     *
     * Deliberately NOT setUserAuthenticationRequired(true): breaks fire on a
     * schedule while the device may be locked and the app in the background,
     * and a key that needs an unlock/biometric first would make the break
     * overlay unable to read or write a reflection exactly then.
     *
     * The IV is generated by the Keystore itself on every encrypt (AES-GCM
     * Keystore keys default to setRandomizedEncryptionRequired(true), which
     * rejects a caller-supplied IV outright) -- so "a fresh random nonce per
     * encryption", the property the whole scheme rests on, is enforced by the
     * platform here rather than by this code remembering to do it. */
    private fun fieldKey(): SecretKey {
        synchronized(fieldKeyLock) {
            val keyStore = KeyStore.getInstance(ANDROID_KEYSTORE).apply { load(null) }
            (keyStore.getKey(FIELD_KEY_ALIAS, null) as? SecretKey)?.let { return it }
            // Only reached when no key exists yet. Generating under the lock,
            // after re-checking, is what keeps two concurrent calls from both
            // generating -- the second would overwrite the first under the
            // same alias and silently strand everything the first encrypted.
    ...
    /** Values come back positionally matched to the input -- Rust rejects a
     * short or long response rather than letting rows shift onto each other
     * (see android_bridge.rs's field_op). */
    @Command
    fun encryptFields(invoke: Invoke) {
        val args = invoke.parseArgs(FieldCryptoArgs::class.java)
        try {
            val key = fieldKey()
            val out = JSArray()
            for (value in args.values) {
                // A fresh Cipher per value: Cipher instances are not
                // thread-safe and carry per-operation IV state.
                val cipher = Cipher.getInstance(FIELD_TRANSFORMATION)
                cipher.init(Cipher.ENCRYPT_MODE, key)
                val ciphertext = cipher.doFinal(value.toByteArray(Charsets.UTF_8))
                val blob = cipher.iv + ciphertext
                out.put(FIELD_MARKER + Base64.encodeToString(blob, Base64.NO_WRAP))
            }
            val ret = JSObject()
            ret.put("values", out)
            invoke.resolve(ret)
        } catch (e: Exception) {
            // Rejected, never resolved with the input untouched: silently
            // handing back plaintext would write unencrypted reflections into
            // a database the rest of the app believes is encrypted.
            Log.e(CRYPTO_TAG, "encryptFields failed", e)
            invoke.reject("encryptFields failed: ${e.message}")
        }
    }

    /** Anything without the marker is returned untouched -- a row written
     * before encryption existed, or one the one-time migration hasn't reached
     * yet. Same passthrough rule crypto.rs applies on desktop. */
    @Command
    fun decryptFields(invoke: Invoke) {
        val args = invoke.parseArgs(FieldCryptoArgs::class.java)
        try {
            val key = fieldKey()
            val out = JSArray()
            for (value in args.values) {
                if (!value.startsWith(FIELD_MARKER)) {
                    out.put(value)
                    continue
                }
                val blob = Base64.decode(value.removePrefix(FIELD_MARKER), Base64.NO_WRAP)
                if (blob.size <= FIELD_IV_BYTES) {
                    throw IllegalArgumentException("ciphertext too short to contain an IV")
                }
                val cipher = Cipher.getInstance(FIELD_TRANSFORMATION)
                cipher.init(
                    Cipher.DECRYPT_MODE,
                    key,
                    GCMParameterSpec(FIELD_TAG_BITS, blob, 0, FIELD_IV_BYTES),
                )
                val plaintext = cipher.doFinal(blob, FIELD_IV_BYTES, blob.size - FIELD_IV_BYTES)
                out.put(String(plaintext, Charsets.UTF_8))
            }
            val ret = JSObject()
            ret.put("values", out)
            invoke.resolve(ret)
